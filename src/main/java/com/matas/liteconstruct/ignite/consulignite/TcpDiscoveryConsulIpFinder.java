package com.matas.liteconstruct.ignite.consulignite;
...
public class TcpDiscoveryConsulIpFinder extends TcpDiscoveryIpFinderAdapter {
  ...
  private void init() {
    ...
    if (consul == null) {
      A.notNullOrEmpty(consulAddres,
          String.format(
              "Consul URL (or system property %s) cannot be null "
                  + "or empty if a Consul client object is not provided explicitly",
              PROP_CONSUL_ADDRESS_KEY));
  ...
  public Collection<InetSocketAddress> getRegisteredAddresses() throws IgniteSpiException {
    ...
    List<ServiceHealth> nodes =
        consul.healthClient().getHealthyServiceInstances(serviceName).getResponse();

    Collection<InetSocketAddress> registeredAddresses = new HashSet<>();

    for (ServiceHealth node : nodes) {
      registeredAddresses
          .add(new InetSocketAddress(node.getService().getAddress(), node.getService().getPort()));
    }
  ...
  public void registerAddresses(Collection<InetSocketAddress> addrs) throws IgniteSpiException {
    ...
    if (log.isInfoEnabled())
      log.info("Registering addresses with Cosnul IP Finder: " + addrs
          + " Addresses that are already registered will be ignored");
    ...
    for (InetSocketAddress addr : addrs) {
      ...
      Registration serviceRegistration = ImmutableRegistration.builder().name(serviceName)
          .address(addr.getAddress().getHostAddress()).port(addr.getPort()).id(serviceUid).build();
  ...
  private String inetAddrToUid(InetSocketAddress addr) {
    return addr.getAddress().getHostName() + ":" + addr.getAddress().getHostAddress() + ":"
        + addr.getPort();
  }
